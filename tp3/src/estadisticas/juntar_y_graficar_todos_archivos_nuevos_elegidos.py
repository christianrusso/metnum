...
def main(argv = sys.argv):
    ...
    for x in archivos_movsgol:
        ...
        if(ret_juntar_todo!=0):
            ...
            sys.exit(1)
        ret_graficos=subprocess.call(["bash","crear_graficos_para_movsgol_nuevos_elegidos.sh",arch+".TODOS"])
        if(ret_graficos!=0):
    ...
    for x in archivos_movstodos:
        ...
        if(ret_juntar_todo!=0):
            print "Falló el juntar_todo.py para el archivo: " + arch
            sys.exit(1)
        ret_graficos=subprocess.call(["bash","crear_graficos_para_movstodos_nuevos_elegidos.sh",arch+".TODOS"])
        if(ret_graficos!=0):
            print "Falló el graficar para el archivo: " + arch
            sys.exit(1)
    for x in archivos_movsestimacion:
        ...
        if(ret_juntar_todo!=0):
            print "Falló el juntar_todo.py para el archivo: " + arch
            sys.exit(1)
        ret_graficos=subprocess.call(["bash","crear_graficos_para_movsestimacion_nuevos_elegidos.sh",arch+".TODOS"])
        if(ret_graficos!=0):
            print "Falló el graficar para el archivo: " + arch
            sys.exit(1)
    return 0
